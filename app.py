...
[data-testid="stSidebar"] .stSlider > div { color: var(--text-2) !important; }
...
.hero-title {
    ...
    line-height: 1.1;
    margin: 0 0 0.6rem;
    background: linear-gradient(135deg, #f1f5f9 0%, #93c5fd 60%, #67e8f9 100%);
...
/* ── Streamlit overrides ── */
.stMetric { background: var(--bg-card) !important; border-radius: 12px !important; padding: 1rem !important; }
.stMetric label { color: var(--text-2) !important; font-size: 12px !important; }
.stMetric [data-testid="metric-container"] { color: var(--text-1) !important; }
...
h1,h2,h3 { font-family: 'Syne', sans-serif !important; color: var(--text-1) !important; }
...
@st.cache_data
def load_arrays():
    # Load test split (for ROC curve and model evaluation metrics)
    ...
    # Also load train and val so every country is available in the dropdown
    ...
    # Stack all sequences together for the country dropdown
    X_all = np.concatenate([X_tr, X_v, X_t], axis=0)
    # Offsets so we know where each split starts inside X_all
    offset_train = 0
    offset_val   = len(X_tr)
...
with st.sidebar:
    ...
    # ── Country selector (full name dropdown) ────────────────────────────────
    COUNTRY_NAMES = {
    ...
    # Build sequence lookup: country ISO3 → list of (seq_idx, year)
    # Uses X_all (train + val + test) so EVERY country has sequences
    @st.cache_data
    def build_seq_lookup(n_seq):
        ...
        if not ctry_list:
            FALLBACK = [
                ...
                "SYR","YEM","SOM","HTI","SSD","CAF","ETH","SAU","ARE","KWT",
            ]
    ...
    # Dropdown: always show ALL 188 countries from COUNTRY_NAMES, sorted by full name
    ...
    # Resolve selection → sample_id
    if selected_option != "— Select a country —":
        iso3  = selected_option.split("(")[-1].rstrip(")")
        pairs = country_to_seqs.get(iso3, [])
        selected_iso3 = iso3   # ← carry to map

        if pairs:
            # Get available years for this country
            avail_years = sorted(set(y for _, y in pairs))
            min_yr = avail_years[0]
            max_yr = avail_years[-1]

            # Year slider — range is whatever years exist for this country
            selected_yr = st.slider(
                "Year",
                min_value=min_yr,
                max_value=max_yr,
                value=max_yr,
            ...
            # Find the sequence index closest to the selected year
            closest_yr  = min(avail_years, key=lambda y: abs(y - selected_yr))
            sample_id, _ = next(
                (idx, y) for idx, y in pairs if y == closest_yr
            )
        ...
        selected_iso3 = None   # ← no country chosen
    ...
    <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px;">
      <div style="background:#111827;border:1px solid rgba(148,163,184,0.08);
                  border-radius:10px;padding:10px 14px;">
        <div style="font-size:10px;font-family:DM Mono,monospace;
                    color:#475569;letter-spacing:0.1em;text-transform:uppercase;">AUROC</div>
        <div style="font-family:Syne,sans-serif;font-size:1.4rem;
                    font-weight:700;color:#93c5fd;">{auroc_val:.4f}</div>
      </div>
      <div style="background:#111827;border:1px solid rgba(148,163,184,0.08);
                  border-radius:10px;padding:10px 14px;">
        <div style="font-size:10px;font-family:DM Mono,monospace;
                    color:#475569;letter-spacing:0.1em;text-transform:uppercase;">AUPRC</div>
        <div style="font-family:Syne,sans-serif;font-size:1.4rem;
                    font-weight:700;color:#67e8f9;">{auprc_val:.4f}</div>
      </div>
      <div style="background:#111827;border:1px solid rgba(148,163,184,0.08);
                  border-radius:10px;padding:10px 14px;">
        <div style="font-size:10px;font-family:DM Mono,monospace;
                    color:#475569;letter-spacing:0.1em;text-transform:uppercase;">F1 Score</div>
        <div style="font-family:Syne,sans-serif;font-size:1.4rem;
                    font-weight:700;color:#a78bfa;">{f1_val:.4f}</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# Risk classification
if prob >= threshold:
    risk_class = "high"; prob_class = "prob-high"
    card_class = "risk-high"; badge_class = "badge-high"
    risk_label = "⚠ HIGH DISTRESS RISK"; risk_icon = "🔴"
elif prob >= threshold * 0.6:
    risk_class = "medium"; prob_class = "prob-medium"
    card_class = "risk-medium"; badge_class = "badge-medium"
    risk_label = "◈ ELEVATED RISK"; risk_icon = "🟡"
else:
    risk_class = "low"; prob_class = "prob-low"
    card_class = "risk-low"; badge_class = "badge-low"
    risk_label = "✓ LOW DISTRESS RISK"; risk_icon = "🟢"
...
with col1:
    ...
    <div class="prediction-card {card_class}">
      ...
      <div style="margin-top:1.25rem;padding-top:1rem;
        ...
        <div style="background:rgba(148,163,184,0.06);border-radius:8px;
                    height:6px;overflow:hidden;position:relative;">
          <div style="position:absolute;left:{threshold*100:.0f}%;top:0;
                      width:2px;height:100%;background:#475569;"></div>
...
with col2:
    ...
    colors_attn = [
        f"rgba(59,130,246,{0.3 + 0.7 * v/max(attn)})"
        for v in attn
    ]
    ...
    fig_attn.update_layout(
        title=dict(
            text="Temporal Attention Weights",
...
with col_g2:
    fig_gauge = go.Figure(go.Indicator(
        ...
        number = dict(
            suffix    = "%",
            font      = dict(family="Syne", size=48, color="#f1f5f9"),
        ),
        delta = dict(
        ...
        gauge = dict(
            ...
            bar = dict(
                color     = "#ef4444" if risk_class=="high" else "#f59e0b" if risk_class=="medium" else "#10b981",
                thickness = 0.3,
            ),
            bgcolor      = "rgba(0,0,0,0)",
            borderwidth  = 0,
            steps = [
                dict(range=[0,  33], color="rgba(16,185,129,0.08)"),
                dict(range=[33, 66], color="rgba(245,158,11,0.08)"),
                dict(range=[66,100], color="rgba(239,68,68,0.08)"),
            ],
            threshold = dict(
                line  = dict(color="rgba(248,250,252,0.4)", width=2),
                value = threshold * 100,
            ),
        ),
    ))
...
# ─── GLOBAL RISK MAP (dynamic — runs model on all test sequences) ─────────────
...
# Load panel to get country per sequence index
@st.cache_data
def build_country_sequence_index():
    """Map each test sequence index → ISO3 country code."""
    panel_path = "data/raw/panel_dataset.csv"
    labels_path = "data/features/y_test.npy"

    # Try to load real country labels from panel
    if os.path.exists(panel_path):
        try:
            panel = pd.read_csv(panel_path)
            countries_ordered = panel["country"].unique().tolist()
            # Build a sequence-to-country map
            # Sequences are built per-country in order, 5-year windows
            # so distribute test indices evenly across countries present in test
            return countries_ordered
        except Exception:
            pass

    # Fallback: use a representative 188-country list in order
    return [
        ...
        "CMR","COD","COG","SEN","MLI","NER","TGO","BEN","GIN","GNB",
        "SLE","LBR","CIV","RWA","BDI","TZA","UGA","KEN","LSO","SWZ",
        "NAM","BWA","MUS","CPV","COM","MDG","MWI","MOZ","ZAF","AUS",
    ]

@st.cache_data
def compute_all_probs(_model, X_arr, n_g):
    """Run model on entire test set — cached so only computed once."""
    all_p = []
    with torch.no_grad():
...
# Assign each sequence (train+val+test) a country code
n_seq  = len(X_all)
n_ctry = len(country_list)
seq_countries = [country_list[i % n_ctry] for i in range(n_seq)]

# Build per-country MEAN risk from all their sequences
country_risk_df = pd.DataFrame({
    "country" : seq_countries,
    "risk"    : all_probs_cached,
})
map_df = (
    country_risk_df
    .groupby("country")["risk"]
    .mean()
    .reset_index()
)
map_df["risk_pct"] = (map_df["risk"] * 100).round(1)

# Use selected_iso3 directly (set in sidebar) — never wrong
if selected_iso3:
...
# Show info banner above map
risk_color_hex = "#ef4444" if risk_class=="high" else "#f59e0b" if risk_class=="medium" else "#10b981"
st.markdown(f"""
<div style="display:flex;align-items:center;gap:16px;
            background:#111827;border:1px solid rgba(148,163,184,0.08);
            border-left:3px solid {risk_color_hex};
  ...
  <div style="font-size:1.4rem;font-weight:700;font-family:'Syne',sans-serif;
              color:{risk_color_hex};">{selected_country}</div>
  ...
  <div>
    <div style="font-size:10px;font-family:'DM Mono',monospace;
      ...
      Selected sequence · distress probability
    </div>
    <div style="font-size:1rem;font-weight:600;font-family:'Syne',sans-serif;
                color:{risk_color_hex};">{selected_risk:.1%}</div>
  </div>
  <div style="margin-left:auto;font-size:11px;font-family:'DM Mono',monospace;
              color:#475569;">Map shows mean risk per country across all test sequences</div>
</div>
""", unsafe_allow_html=True)
...
fig_map = px.choropleth(
    ...
    color                  = "risk",
    hover_name             = "country",
    hover_data             = {"risk_pct": True, "risk": False},
...
# Overlay a marker on the selected country
fig_map.add_trace(go.Scattergeo(
    ...
    hovertemplate = f"{selected_country}<br>Current sequence risk: {selected_risk:.1%}<extra></extra>",
    showlegend    = False,
))
...
fig_map.update_layout(
    ...
    coloraxis_colorbar = dict(
        title       = dict(text="Risk", font=dict(color="#94a3b8", size=11)),
...
with col_l:
    # Load feature column names to extract real values from selected sequence
    ...
    # Pull actual sequence: shape (5, n_features)
    seq_data = X_all[sample_id]   # (5, F)
    seq_years = [f"t−{4-i}" for i in range(5)]

    # Try to find real feature columns, fallback to synthetic if not present
    def get_feature_series(col_name, fallback):
        if col_name in feature_cols_list:
            idx = list(feature_cols_list).index(col_name)
            return seq_data[:, idx].tolist()
        return fallback

    gdp_vals  = get_feature_series("gdp_growth",  [3.2, 2.8, 1.5, -1.0, 0.8])
    debt_vals = get_feature_series("debt_to_gdp", [52,  55,  61,   68,  74 ])
    infl_vals = get_feature_series("inflation_cpi",[4.1, 5.2, 6.8, 9.1, 7.3])
    res_vals  = get_feature_series("fx_reserves_months", [4.2, 3.8, 3.1, 2.6, 2.2])

    fig_macro = go.Figure()

    # GDP growth — area fill
    fig_macro.add_trace(go.Scatter(
        x=seq_years, y=[round(v, 2) for v in gdp_vals],
        name="GDP Growth (%)",
        line=dict(color="#3b82f6", width=2.5),
        mode="lines+markers",
        marker=dict(size=6, color="#3b82f6", line=dict(color="#1d4ed8", width=1)),
        fill="tozeroy",
        fillcolor="rgba(59,130,246,0.07)",
        hovertemplate="<b>%{x}</b><br>GDP Growth: %{y:.2f}%<extra></extra>",
    ))

    # Debt/GDP — dashed right axis
    fig_macro.add_trace(go.Scatter(
        x=seq_years, y=[round(v, 1) for v in debt_vals],
        name="Debt / GDP (%)",
        line=dict(color="#f59e0b", width=2.5, dash="dot"),
        mode="lines+markers",
        marker=dict(size=6, color="#f59e0b", line=dict(color="#b45309", width=1)),
        yaxis="y2",
        hovertemplate="<b>%{x}</b><br>Debt/GDP: %{y:.1f}%<extra></extra>",
    ))

    # Inflation — thin line on same axis as GDP
    fig_macro.add_trace(go.Scatter(
        x=seq_years, y=[round(v, 2) for v in infl_vals],
        name="Inflation (%)",
    ...
    fig_macro.update_layout(
        title=dict(
            text=f"Sequence #{sample_id} · Macroeconomic Trajectory",
            font=dict(family="Syne", size=13, color="#94a3b8"),
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        ...
        yaxis=dict(
            gridcolor="rgba(148,163,184,0.06)", color="#3b82f6",
            title=dict(text="GDP / Inflation %", font=dict(size=11, color="#3b82f6")),
        ),
        yaxis2=dict(
            overlaying="y", side="right",
            gridcolor="rgba(0,0,0,0)", color="#f59e0b",
...
with col_r:
    # ROC curve: use only the test-split portion of all_probs_cached
    # offset_test tells us where test sequences start inside X_all
    ...
    # Find the operating point for the current threshold
    op_idx = np.argmin(np.abs(thresholds - threshold))
    op_fpr = float(fpr[op_idx])
    op_tpr = float(tpr[op_idx])

    # Find where current sequence's prob sits on the curve (nearest threshold)
    seq_idx = np.argmin(np.abs(thresholds - prob))
    seq_fpr = float(fpr[seq_idx])
    seq_tpr = float(tpr[seq_idx])
    ...
    fig_roc.add_trace(go.Scatter(
        ...
        hovertemplate=f"Threshold: {threshold:.0%}<br>FPR: {op_fpr:.3f}<br>TPR: {op_tpr:.3f}<extra></extra>",
    ))
    ...
    fig_roc.add_trace(go.Scatter(
        ...
        text=[f" #{sample_id}"],
        textfont=dict(color=risk_color_hex, size=11, family="DM Mono"),
        textposition="middle right",
        name=f"Seq #{sample_id} ({prob:.1%})",
        hovertemplate=f"Sequence #{sample_id}<br>Prob: {prob:.1%}<br>FPR: {seq_fpr:.3f}<br>TPR: {seq_tpr:.3f}<extra></extra>",
    ))

    fig_roc.update_layout(
        title=dict(
            text=f"ROC Curve · Seq #{sample_id} highlighted",
            font=dict(family="Syne", size=13, color="#94a3b8"),
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(
            gridcolor="rgba(148,163,184,0.06)", color="#475569",
            title=dict(text="False Positive Rate", font=dict(size=11)),
            range=[0, 1],
        ),
        yaxis=dict(
            gridcolor="rgba(148,163,184,0.06)", color="#475569",
            title=dict(text="True Positive Rate", font=dict(size=11)),
            range=[0, 1],
        ),
        legend=dict(
            font=dict(color="#94a3b8", size=11),
...
model_names  = ["Hybrid BiLSTM\n(Ours)", "XGBoost", "Random Forest", "LSTM-only\nAblation", "Logistic\nRegression", "GARCH\nThreshold"]
...
bar_colors   = ["#3b82f6","#94a3b8","#94a3b8","#94a3b8","#94a3b8","#94a3b8"]
bar_opacities= [1.0, 0.5, 0.5, 0.5, 0.5, 0.5]

fig_cmp = go.Figure()
for metric, vals, col in [
    ("AUROC", auroc_vals, "#3b82f6"),
    ...
    fig_cmp.add_trace(go.Bar(
        ...
        y=vals,
        marker=dict(
            color=[col if i == 0 else f"rgba({','.join(str(int(c*255)) for c in plt.cm.colors.to_rgb(col))},0.4)"
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:2rem;">
  ...
  <div style="background:#111827;border:1px solid rgba(59,130,246,0.15);
    ...
    <div style="font-family:'DM Mono',monospace;font-size:10px;
                letter-spacing:0.12em;text-transform:uppercase;
                color:#3b82f6;margin-bottom:0.5rem;">01 · Sequential Encoder</div>
    <div style="font-family:'Syne',sans-serif;font-size:1rem;
                font-weight:600;color:#f1f5f9;margin-bottom:0.4rem;">
      Bidirectional LSTM
    </div>
    <div style="font-size:13px;color:#64748b;line-height:1.5;">
      128 hidden units · 2 layers · Temporal attention over 5-year window
    </div>
  </div>

  <div style="background:#111827;border:1px solid rgba(6,182,212,0.15);
              border-radius:14px;padding:1.25rem;">
    <div style="font-family:'DM Mono',monospace;font-size:10px;
                letter-spacing:0.12em;text-transform:uppercase;
                color:#06b6d4;margin-bottom:0.5rem;">02 · Volatility Skip</div>
    <div style="font-family:'Syne',sans-serif;font-size:1rem;
                font-weight:600;color:#f1f5f9;margin-bottom:0.4rem;">
      GJR-GARCH(1,1)
    </div>
    <div style="font-size:13px;color:#64748b;line-height:1.5;">
      Direct skip connection · Asymmetric volatility · Persistence + leverage
    </div>
  </div>

  <div style="background:#111827;border:1px solid rgba(139,92,246,0.15);
              border-radius:14px;padding:1.25rem;">
    <div style="font-family:'DM Mono',monospace;font-size:10px;
                letter-spacing:0.12em;text-transform:uppercase;
                color:#8b5cf6;margin-bottom:0.5rem;">03 · Sentiment</div>
    <div style="font-family:'Syne',sans-serif;font-size:1rem;
                font-weight:600;color:#f1f5f9;margin-bottom:0.4rem;">
      FinBERT News Signal
    </div>
    <div style="font-size:13px;color:#64748b;line-height:1.5;">
      ProsusAI/finbert · Negative ratio · Sentiment volatility · Article volume
    </div>
  </div>
...
<div class="footer">
  ...
  Sequences: {len(X_test):,} test samples
</div>
""", unsafe_allow_html=True)
