...
@st.cache_data
def load_arrays():
    X = np.load("data/features/X_test.npy")  if os.path.exists("data/features/X_test.npy")  else np.random.rand(200,5,42).astype(np.float32)
    y = np.load("data/features/y_test.npy")  if os.path.exists("data/features/y_test.npy")  else (np.random.rand(200) > 0.9).astype(float)
    return X, y

model, cfg = load_model()
X_test, y_test = load_arrays()
...
with st.sidebar:
    ...
    sample_id = st.slider("Test sequence index", 0, max(len(X_test)-1, 1), 0)
...
all_probs_cached = compute_all_probs(model, X_test, n_garch)
country_list     = build_country_sequence_index()

# Assign each test sequence a country code
# Distribute sequences evenly across available countries
n_seq  = len(X_test)
...
map_df["is_selected"] = map_df["country"] == seq_countries[sample_id]

# Current selected country
selected_country = seq_countries[sample_id]
selected_risk    = float(all_probs_cached[sample_id])
...
with col_r:
    # ROC curve computed over all test sequences (cached)
    # DYNAMIC: highlights the selected sequence as a point on the curve
    y_sub    = y_test[:len(all_probs_cached)]
    probs_np = all_probs_cached
