...
# Representative risk scores anchored to known real-world situations
map_countries = [
    ...
    "SAU","ARE","KWT","QAT","OMN","RUS",
]
map_risk = [
    0.88, 0.95, 0.82, 0.79, 0.75, 0.92, 0.97, 0.96, 0.98, 0.94,
    0.85, 0.78, 0.72, 0.65, 0.60, 0.58, 0.80, 0.70, 0.88, 0.90,
    0.76, 0.55, 0.68, 0.72, 0.85, 0.77, 0.65, 0.52, 0.62, 0.55,
    0.70, 0.48, 0.38, 0.32, 0.35, 0.40, 0.22, 0.18, 0.25, 0.28,
    0.20, 0.18, 0.25, 0.35, 0.30, 0.42, 0.28, 0.20, 0.32, 0.30,
    0.15, 0.10, 0.12, 0.08, 0.09, 0.07, 0.10, 0.11, 0.06, 0.05,
    0.22, 0.18, 0.15, 0.12, 0.20, 0.72,
]

map_df = pd.DataFrame({"country": map_countries, "risk": map_risk})
...
with col_l:
    ...
    gdp   = [3.2, 3.5, 2.8, 1.5, -3.0, 4.1, 3.8, 2.9, 2.1, 1.9, 1.5]
    debt  = [45,  48,  50,  55,   62,  68,  70,  72,  75,  80,  83 ]
    ...
    fig_macro.add_trace(go.Scatter(
        ...
        hovertemplate="%{y:.1f}%<extra>GDP Growth</extra>",
    ))
    fig_macro.add_trace(go.Scatter(
        x=years, y=debt,
        name="Debt / GDP (%)",
        ...
        yaxis="y2",
        hovertemplate="%{y:.0f}%<extra>Debt/GDP</extra>",
    ))
    fig_macro.update_layout(
        title=dict(text="Macroeconomic Indicators",
                   font=dict(family="Syne", size=13, color="#94a3b8")),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        legend=dict(
        ...
        yaxis=dict(
            ...
            title=dict(text="GDP Growth %", font=dict(size=11, color="#3b82f6")),
        ),
        yaxis2=dict(
...
with col_r:
    ...
    with torch.no_grad():
        for i in range(min(len(X_test), 500)):
            ...
            if model is not None:
                lg, _ = model(x, gf)
                probs_all.append(torch.sigmoid(lg).item())
            else:
                probs_all.append(float(np.random.rand()))

    y_sub   = y_test[:len(probs_all)]
    probs_np = np.array(probs_all)
    fpr, tpr, _ = roc_curve(y_sub, probs_np)
    roc_auc = auc(fpr, tpr)
